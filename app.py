    ...
    if request.method == "POST":
        ...
        if not groq_key:
            ...
            if err:
                ...
                if query:
                    try:
                        # 1. Build the prompt that turns a plain-English question
                        #    into a request for pandas code
                        prompt = f"""
...
"""
                        # 2. Ask the LLM (via Groq) to generate the code
                        ...
                        # 3. Run the generated code against the real DataFrame
                        local_vars = {"df": df}
